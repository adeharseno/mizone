...
class preparations extends Controller {
	protected $dir = "cw-admin/contents/preparations/";
	protected $title = "Preparations";
	...
	public function save($parent_id, $id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if ($id){
				$this->db->update("preparations", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("preparations", $insert);	
			...
			redirect(PATH_CW .  'preparations/edit/'. $parent_id /*. "/" . $id*/,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("preparations", $set, $where);
		
		$insert["set"] = $set;
		$insert["where"] = $where;
		
		$parent_id = $this->db->select("*")->from("preparations")->where($where)->get()->row()->recipes_id;
