...
class distributors extends CI_Controller {
	protected $dir = "cw-admin/contents/distributors/";
	protected $title = "Distributors";
	...
	public function save($id)
	{
		...
		$this->form_validation->set_rules('name[id]', 'Title', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$insert["name"] = json_encode($data_post["name"]);
			$insert["telephone"] = json_encode($data_post["telephone"]);
			$insert["fax"] = json_encode($data_post["fax"]);
			$insert["email"] = json_encode($data_post["email"]);
			$insert["area"] = json_encode($data_post["area"]);
			$insert["address"] = json_encode($data_post["address"]);
			
			
			
			$insert["distributors_sub_category_id"] = $data_post["distributors_sub_category_id"];
			$insert["flag"] = $data_post["flag"];

			// debug_pre($insert);exit();
			
			if ($id){
				$this->db->update("distributors", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("distributors", $insert);	
				$id = $this->db->insert_id();
				set_flashdata("success_message", "Save Success");
			}	
			
			redirect(PATH_CW .  'distributors/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("distributors", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "distributors";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'distributors');
		}
