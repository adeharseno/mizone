...
class distributors_category extends CI_Controller {
	protected $dir = "cw-admin/contents/distributors_category/";
	protected $title = "Distributors Category";
	...
	public function save($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if ($id){
				$this->db->update("distributors_category", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("distributors_category", $insert);	
				$id = $this->db->insert_id();
				set_flashdata("success_message", "Save Success");
			}	
			
			redirect(PATH_CW .  'distributors_category/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("distributors_category", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "distributors_category";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'distributors_category');
		}
