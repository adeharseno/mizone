...
class machines_category extends CI_Controller {
	protected $dir = "cw-admin/contents/machines_category/";
	protected $title = "Machines Category";
	...
	public function save($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$insert["icon"] = $this->image_path->get_id($data_post["icon"]);
			$insert["flag"] = $data_post["flag"];

			
			
			if ($id){
				$this->db->update("machines_category", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("machines_category", $insert);	
				$id = $this->db->insert_id();
				set_flashdata("success_message", "Save Success");
			}	
			
			redirect(PATH_CW .  'machines_category/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("machines_category", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "machines_category";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'machines_category');
		}
