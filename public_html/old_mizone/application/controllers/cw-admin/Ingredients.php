...
class ingredients extends CI_Controller {
	protected $dir = "cw-admin/contents/ingredients/";
	...
	public function save($parent_id, $id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$insert["measurement"] = json_encode($data_post["measurement"]);
			
			// $insert["order_by"] = json_encode($data_post["order_by"]);
			// $insert["order_by"] = $data_post["order_by"];
			$insert["flag"] = $data_post["flag"];
			$insert["recipes_id"] = $parent_id;

			if ($id){
				$this->db->update("ingredients", $insert, array("id"=>$id));
			...
			redirect(PATH_CW .  'ingredients/edit/'. $parent_id /*. "/" . $id*/,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("ingredients", $set, $where);
		
		$insert["set"] = $set;
		$insert["where"] = $where;
		
		$parent_id = $this->db->select("*")->from("ingredients")->where($where)->get()->row()->recipes_id;

		set_flashdata("success_message", ucwords(str_replace("_"," ",__class__)) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "recipes/edit/" . $parent_id;
			echo json_encode($json);
		}else{
			redirect(PATH_CW . "recipes/edit/" . $parent_id);
		}
