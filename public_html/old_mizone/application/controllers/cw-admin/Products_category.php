...
class products_category extends CI_Controller {
	protected $dir = "cw-admin/contents/products_category/";
	protected $title = "Products Category";
	...
	public function save($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if ($id){
				$this->db->update("products_category", $insert, array("id"=>$id));
			...
			redirect(PATH_CW .  'products_category/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("products_category", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "products_category";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'products_category');
		}
