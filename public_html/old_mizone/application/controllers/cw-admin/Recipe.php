...
class Recipe extends CI_Controller {
	protected $dir = "cw-admin/contents/recipe/";
	...
	public function save($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$insert["informasi_resep"] = json_encode($data_post["informasi_resep"]);
			$insert["ingredients"] = json_encode($data_post["ingredients"]);
			$insert["how_to"] = json_encode($data_post["how_to"]);
			...
			if ($id){
				$this->db->update("recipe", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("recipe", $insert);	
				$id = $this->db->insert_id();
				set_flashdata("success_message", "Save Success");
			}	
			
			redirect(PATH_CW .  'recipe/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("recipe", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "recipe";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'recipe');
		}
