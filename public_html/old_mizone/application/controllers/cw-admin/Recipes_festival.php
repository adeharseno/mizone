...
class recipes_festival extends CI_Controller {
	protected $dir = "cw-admin/contents/recipes_festival/";
	protected $title = "Recipes Festival";
	...
	public function save($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if ($id){
				$this->db->update("recipes_festival", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("recipes_festival", $insert);	
				$id = $this->db->insert_id();
				set_flashdata("success_message", "Save Success");
			}	
			
			redirect(PATH_CW .  'recipes_festival/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("recipes_festival", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "recipes_festival";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'recipes_festival');
		}
