...
class recipes_sub_category extends CI_Controller {
	protected $dir = "cw-admin/contents/recipes_sub_category/";
	protected $title = "Recipes Sub Category";
	...
	public function show($parent_id, $id)
	{
		...
		if ($id) {
			$where = null;
			$where["recipes_category_id"] = $parent_id;
			$where["id"] = $id;
			$row = $this->db->select("*")
	...
	public function save($parent_id, $id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$insert["recipes_category_id"] = $parent_id;

			if ($id){
				$this->db->update("recipes_sub_category", $insert, array("id"=>$id));
				set_flashdata("success_message", "Update Success");
			}else {
				$insert["created_at"] = date('Y-m-d H:i:s');
				$this->db->insert("recipes_sub_category", $insert);	
			...
			redirect(PATH_CW .  'recipes_sub_category/edit/'. $parent_id /*. "/" . $id*/,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("recipes_sub_category", $set, $where);
		
		$insert["set"] = $set;
		$insert["where"] = $where;
		
		$parent_id = $this->db->select("*")->from("recipes_sub_category")->where($where)->get()->row()->recipes_category_id;

		set_flashdata("success_message", ucwords(str_replace("_"," ",__class__)) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "recipes_category/edit/" . $parent_id;
			echo json_encode($json);
		}else{
			redirect(PATH_CW . "recipes_category/edit/" . $parent_id);
		}
	...
	public function order()
	{
		...
		$json["success"] = true;
		$json["redirect"] = "recipes_sub_category";
		echo json_encode($json);
		// $this->action("blocked", $id, true);
	}
}
