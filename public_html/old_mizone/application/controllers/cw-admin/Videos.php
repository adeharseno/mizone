...
class Videos extends CI_Controller {
	protected $dir = "cw-admin/contents/videos/";
	...
	public function save($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if ($id){
				$this->db->update("videos", $insert, array("id"=>$id));
			...
			redirect(PATH_CW .  'videos/edit/' . $id,'refresh');
    	}
	}
	...
	public function action($type, $id, $ajax = false)
	{
		...
		$this->db->update("videos", $set, $where);

		set_flashdata("success_message", ucwords(__class__) . " saved");

		if ( $ajax ){
			$json["success"] = true;
			$json["redirect"] = "videos";
			echo json_encode($json);
		}else{
			redirect(PATH_CW . 'videos');
		}
